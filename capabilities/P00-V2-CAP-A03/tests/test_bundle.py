...
A02_BUNDLE = BUNDLE.parent / "P00-V2-CAP-A02"
BUILD = BUNDLE / "build"
...
class CapabilityBundleTests(unittest.TestCase):
    ...
    def test_registry_is_closed_and_admits_only_p01(self) -> None:
        ...
        self.assertEqual(len({phase["plan"] for phase in registry["phases"]}), 1)

    def test_request_schema_is_closed_and_p01_only(self) -> None:
    ...
    def test_rendered_controller_is_narrow_and_complete(self) -> None:
        ...
        self.assertIn('"size": Int(info.st_size)', source)
        self.assertIn('"flags": Int(info.st_flags)', source)
        self.assertIn('"mtimeSeconds": Int(info.st_mtimespec.tv_sec)', source)
        self.assertIn('"ctimeNanoseconds": Int(info.st_ctimespec.tv_nsec)', source)
        self.assertIn("lhs.st_flags == rhs.st_flags", source)
        self.assertIn("func publishRunEvidence", source)
        self.assertIn("root-owned-read-only-top-level-P01-v1", source)
        self.assertIn("try publishRunEvidence(runDirectory)", source)
        for phase in PHASES:
    ...
    def test_complete_payload_manifest_and_envelope(self) -> None:
        ...
        envelope = json.loads((BUILD / "release-envelope.json").read_text())
        self.assertEqual(envelope["artifactId"], "P00-V2-CAP-A03")
        self.assertEqual(
    ...
    def install_a02(self, test_root: pathlib.Path) -> subprocess.CompletedProcess[str]:
        environment = os.environ.copy()
        environment["P00_V2_TEST_ROOT"] = str(test_root)
        environment["P00_V2_TEST_ALLOW_FRESH_INSTALL"] = "1"
        result = run(
            "/bin/zsh",
            A02_BUNDLE / "source/install.sh",
            A02_BUNDLE,
            sha256(A02_BUNDLE / "build/release-envelope.json"),
            env=environment,
            check=False,
        )
    ...
    def test_atomic_upgrade_from_exact_a02_preserves_state(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_text:
            temporary = pathlib.Path(temporary_text)
            test_root = self.prepare_legacy_root(temporary)
            try:
                installed_a02 = self.install_a02(test_root)
                self.assertEqual(installed_a02.returncode, 0, installed_a02.stderr)
                controller = (
                    test_root
                ...
                historical_evidence = historical_run / "failure.json"
                historical_evidence.write_text('{"preserved":true}\n')
                historical_evidence.chmod(0o600)
                ...
                self.assertEqual(
                    stat.S_IMODE(historical_evidence.stat().st_mode), 0o444
                )
                self.assertEqual(
                    historical_evidence.read_text(), '{"preserved":true}\n'
                )
                self.assertEqual(
                    {
                        path.name: (
                ...
                self.assertFalse(
                    any(controller.glob(".v2-before-P00-V2-CAP-A03.*"))
                )
                rendered = (
    ...
    def test_upgrade_failure_restores_exact_a02_without_authorization(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_text:
            temporary = pathlib.Path(temporary_text)
            test_root = self.prepare_legacy_root(temporary)
            try:
    ...
    def test_upgrade_rejects_incomplete_a02_identity(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_text:
            temporary = pathlib.Path(temporary_text)
            test_root = self.prepare_legacy_root(temporary)
            try:
                ...
                self.assertFalse((controller / "metadata/P00-V2-CAP-A03").exists())
                self.assertEqual(
                    sha256(controller / "v2/libexec/verify-phase-core"),
                    sha256(A02_BUNDLE / "build/controller"),
                )
            finally:
                make_writable(temporary)

    def test_upgrade_rejects_held_a02_phase_lock(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_text:
            temporary = pathlib.Path(temporary_text)
            test_root = self.prepare_legacy_root(temporary)
            try:
    ...
    def test_upgrade_rolls_back_if_quiescence_holder_dies(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_text:
            ...
            try:
                ...
                run(
                    "/usr/bin/python3",
                    A02_BUNDLE / "tools/manifest.py",
                    "verify",
                    controller / "v2",
                    A02_BUNDLE / "build/expected-install-manifest.json",
                    "--allow-source-provenance",
                )
    ...
    def test_a03_rejects_fresh_v1_path(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_text:
            temporary = pathlib.Path(temporary_text)
            test_root = self.prepare_legacy_root(temporary)
            environment = os.environ.copy()
            ...
            self.assertNotEqual(rejected.returncode, 0)
            self.assertIn(
                "A03 requires the exact installed A02 capability",
                rejected.stderr,
            )
            self.assertFalse(
                (
        ...
        for variable in [
            ...
            with self.subTest(variable=variable), tempfile.TemporaryDirectory() as temporary_text:
                ...
                try:
                    installed_a02 = self.install_a02(test_root)
                    self.assertEqual(
                        installed_a02.returncode, 0, installed_a02.stderr
                    )
    ...
    def test_revocation_metadata_drift_removes_authorization_first(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_text:
            ...
            try:
                ...
                metadata = controller / "metadata/P00-V2-CAP-A03"
                make_writable(metadata)
            ...
            try:
                ...
                preserved = list(
                    (
                        test_root
                        / "Users/Shared/InterviewCopilot/revocation-receipts"
                    ).glob("P00-V2-CAP-A03-sudoers.removed.*")
                )
