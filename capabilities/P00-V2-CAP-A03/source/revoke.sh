...
removed_sudoers=$receipt_root/P00-V2-CAP-A03-sudoers.removed.$$
...
if [[ ! -e "$install_root" ]]; then
  print "P00-V2-CAP-A03 already revoked"
  exit 0
fi
...
retention_root=$receipt_root/P00-V2-CAP-A03-evidence
[[ ! -e "$retention_root" ]] || {
...
if [[ -n "$test_root" ]]; then
  for removable in "$install_root" "$controller_root/objects" \
    "$controller_root/anchors" "$controller_root/runs" "$controller_root/receipts" \
    "$controller_root/locks" \
    "$controller_root/requests" "$controller_root/nonces"; do
...
receipt=$receipt_root/P00-V2-CAP-A03-revoked.json
temporary=$receipt.tmp.$$
retention_sha=$(hash_file "$retention_index")
print -r -- \
  "{\"artifactId\":\"P00-V2-CAP-A03\",\"envelopeSha256\":\"$approved_envelope_sha\",\"authorizationRemovedFirst\":true,\"retentionIndexSha256\":\"$retention_sha\"}" \
  > "$temporary"
/bin/chmod 0444 "$temporary"
[[ -n "$test_root" ]] || /usr/sbin/chown root:wheel "$temporary"
/bin/mv "$temporary" "$receipt"

for forbidden in "$sudoers_target" "$install_root"; do
  [[ ! -e "$forbidden" ]] || {
...
[[ -d "$legacy_root" ]] || {
  print -u2 "revocation did not preserve quarantined v1"
  exit 74
}
print "P00-V2-CAP-A03 revoked; authorization removed, v1 quarantined, evidence retained"
