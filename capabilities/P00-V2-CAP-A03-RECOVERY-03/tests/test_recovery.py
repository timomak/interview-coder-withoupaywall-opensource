...
ARTIFACT = "P00-V2-CAP-A03-RECOVERY-03"
...
class RecoveryTests(unittest.TestCase):
    ...
    def test_envelope_and_handoff_are_closed(self) -> None:
        ...
        self.assertIn("tools/child_runner.py", envelope["members"])
        self.assertIn("tools/a03_state.py", envelope["members"])
        ...
        self.assertIn("P00-V2-CAP-A03-RECOVERY-03.child.lock", handoff)
        self.assertIn("P00-V2-CAP-A03-RECOVERY-03.snapshot.lock", handoff)
        self.assertLess(
            handoff.index("trap cleanup EXIT"),
            handoff.index('/bin/mkdir -p "$stage"'),
        )
        self.assertLess(
    ...
    def test_child_fork_before_pid_journal_is_lock_serialized_and_replay_safe(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_text:
            temporary = pathlib.Path(temporary_text)
            test_root = self.prepare_legacy_root(temporary)
            ready = temporary / "child-before-pid.ready"
            release = temporary / "child-before-pid.release"
            first: subprocess.Popen[str] | None = None
            try:
                self.make_unauthorized_a02(test_root)
                first = self.recover_popen(
                    test_root,
                    P00_V2_RECOVERY_TEST_CHILD_BEFORE_PID_READY=str(ready),
                    P00_V2_RECOVERY_TEST_CHILD_BEFORE_PID_RELEASE=str(release),
                )
                ...
                while not ready.exists():
                    if first.poll() is not None:
                        ...
                        self.fail(
                            "child did not reach the pre-PID lock boundary; "
                            f"exit={first.returncode}; stdout={stdout}; stderr={stderr}"
                        )
                    if time.monotonic() >= deadline:
                        self.fail("timed out waiting for child pre-PID lock boundary")
                    time.sleep(0.01)

                first.kill()
                first.wait(timeout=10)
                blocked = self.recover(test_root)
                self.assertEqual(blocked.returncode, 75, blocked.stderr)
                self.assertIn("lifecycle lock is still active", blocked.stderr)
                self.assertFalse(self.sudoers(test_root).exists())
                self.assertEqual(
                    sha256(self.controller(test_root) / "v2/libexec/verify-phase-core"),
                    sha256(A02 / "build/controller"),
                ...
                lock = (
                    self.controller(test_root)
                    / f"recovery-journals/{ARTIFACT}.child.lock"
                )
                while True:
                    probe = run(
                        "/usr/bin/python3",
                        BUNDLE / "tools/child_runner.py",
                        "probe",
                        lock,
                        check=False,
                    )
                    if probe.returncode == 0:
                        break
                    if time.monotonic() >= deadline:
                        self.fail("child lifecycle lock did not release")
                    time.sleep(0.01)
    ...
    def test_partial_rollback_snapshot_is_not_published_and_replays(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_text:
            temporary = pathlib.Path(temporary_text)
            test_root = self.prepare_legacy_root(temporary)
            try:
                ...
                crashed = self.recover(
                    test_root,
                    P00_V2_RECOVERY_TEST_HARD_CRASH_POINT="mid-rollback-snapshot",
                )
                self.assertNotEqual(crashed.returncode, 0)
                quarantine = self.controller(test_root) / f"quarantine/{ARTIFACT}"
                self.assertFalse((quarantine / "a02-rollback").exists())
                partials = list(quarantine.glob(".a02-rollback-stage.*"))
    ...
    def test_incomplete_a03_metadata_never_qualifies_as_committed(self) -> None:
        for drift in ("extra-member", "invalid-self-test"):
            with self.subTest(drift=drift):
                with tempfile.TemporaryDirectory() as temporary_text:
                    ...
                    try:
                        ...
                        metadata = (
                            self.controller(test_root)
                            / "metadata/P00-V2-CAP-A03"
                        )
                        if drift == "extra-member":
                            (metadata / "unexpected").write_text("not closed\n")
                        else:
                            evidence = metadata / "installed-self-test.json"
                            evidence.chmod(0o644)
                            document = json.loads(evidence.read_text())
                            document["negativeControl"]["aggregateExit"] = 0
                        ...
                        self.assert_failed_and_restored(test_root, replay)
