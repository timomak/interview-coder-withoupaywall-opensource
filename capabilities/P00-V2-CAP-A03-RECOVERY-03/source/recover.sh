...
a02_rollback_stage=$quarantine_root/.a02-rollback-stage.$$
...
child_lock=$journal_root/$artifact_id.child.lock
snapshot_lock=$journal_root/$artifact_id.snapshot.lock
...
child_runner_tool=$bundle_root/tools/child_runner.py
a03_state_tool=$bundle_root/tools/a03_state.py
...
a03_is_exact_without_authorization() {
  [[ -d "$install_root" && -d "$a03_metadata" &&
      -d "$a02_rollback" &&
      "$(hash_file "$install_root/libexec/verify-phase-core")" ==
        "$expected_a03_controller_sha" ]] || return 1
  /usr/bin/python3 "$a03_manifest_tool" verify \
  ...
  /usr/bin/python3 "$a03_manifest_tool" verify \
    ...
    >/dev/null 2>&1 || return 1
  /usr/bin/python3 "$a03_state_tool" \
    "$a03_metadata" "$state_uid" "$state_gid" \
...
has_partial_snapshots() {
  setopt local_options null_glob
  local partials=("$quarantine_root"/.a02-rollback-stage.*)
  (( ${#partials[@]} > 0 ))
}
...
rollback_child_to_a02() {
  setopt local_options null_glob
  local partial
  for partial in "$quarantine_root"/.a02-rollback-stage.*; do
    [[ -d "$partial" && ! -L "$partial" ]] || return 74
    /bin/chmod -R u+w "$partial" 2>/dev/null || true
    /bin/rm -rf "$partial"
  done
...
remove_journal() {
  ...
  for lifecycle_lock in "$child_lock" "$snapshot_lock"; do
    if [[ -e "$lifecycle_lock" ]]; then
      if /usr/bin/python3 "$child_runner_tool" probe "$lifecycle_lock"; then
        /bin/rm -f "$lifecycle_lock"
      else
        return 75
      fi
    fi
...
cleanup() {
  ...
  trap - EXIT
  if (( committed == 0 )); then
    if (( defer_active_child == 1 )); then
      # The active child may still execute sealed members from this exact
      # stage. Preserve it for the next replay to reconcile.
      cleanup_stage=0
      exit "$result"
    fi
...
if [[ -e "$journal_path" ]]; then
  ...
  attempt_token=$(
    /usr/bin/python3 "$journal_tool" rotate \
      "$journal_path" "$approved_envelope_sha"
  )
  if ! /usr/bin/python3 "$child_runner_tool" probe "$child_lock" ||
    ! /usr/bin/python3 "$child_runner_tool" probe "$snapshot_lock"; then
    defer_active_child=1
    print -u2 "A03 child lifecycle lock is still active"
    exit 75
  fi
  ...
  if [[ -d "$a02_rollback" || -f "$runs_snapshot" ]] ||
    has_partial_snapshots; then
    rollback_child_to_a02
  fi
  /usr/bin/python3 "$run_state_tool" reconcile-relocation \
    "$historical_node_modules" "$quarantined_node_modules" \
...
[[ ! -e "$a02_rollback" && ! -e "$a02_rollback_stage" ]] || {
  print -u2 "A02 rollback snapshot path is not fresh"
  exit 73
}
if [[ -n "$test_root" &&
      "${P00_V2_RECOVERY_TEST_HARD_CRASH_POINT:-}" == \
        "mid-rollback-snapshot" ]]; then
  /bin/mkdir -p "$a02_rollback_stage/config"
  /usr/bin/install -m 0444 "$install_root/config/npmrc" \
    "$a02_rollback_stage/config/npmrc"
  hard_crash mid-rollback-snapshot
fi
/usr/bin/python3 "$child_runner_tool" snapshot \
  "$snapshot_lock" "$journal_path" "$approved_envelope_sha" "$attempt_token" \
  "$journal_tool" "$install_root" "$a02_rollback_stage"
/usr/bin/python3 "$a03_manifest_tool" verify \
  "$a02_rollback_stage" "$a02_manifest" "${verify_options[@]}"
/bin/mv "$a02_rollback_stage" "$a02_rollback"
/usr/bin/python3 "$a03_manifest_tool" verify \
  "$a02_rollback" "$a02_manifest" "${verify_options[@]}"
/usr/bin/python3 "$journal_tool" set "$journal_path" \
...
/usr/bin/python3 "$child_runner_tool" run \
  "$child_lock" "$journal_path" "$approved_envelope_sha" "$attempt_token" \
  "$journal_tool" "$a03_installer" "$a03_root" "$expected_a03_envelope_sha"
