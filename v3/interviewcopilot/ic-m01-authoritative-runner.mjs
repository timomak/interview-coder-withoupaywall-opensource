...
const EXPECTED_CANDIDATE = "703a37e47b4a398ed90e1c5cf4be9c5859dc0849"
const ATTEMPT_ID = "IC-M01-AUTH-02"
...
if (!repositoryRoot || !evidenceRoot) {
  throw new Error("usage: node ic-m01-authoritative-runner.mjs REPOSITORY EVIDENCE_ROOT")
}
...
const bindingHash = sha256(
  canonicalJson({
    candidate,
    protocol: "interviewcopilot-local-authoritative-v1",
    runId,
    suiteLabels: [
      "install",
      "policy",
...
runTest("unit", "test:unit", 85)
runTest("p01", "test:p01", 27)
runTest("p02", "test:p02", 11)
runTest("p03", "test:p03", 14)
runTest("p04", "test:p04", 24)
...
fs.writeFileSync(
  ...
  `${canonicalJson({
    schemaVersion: 1,
    protocol: "interviewcopilot-local-authoritative-result-v1",
    status: "PASS",
    runId,
    candidate,
    bindingHash,
