...
def wrong_group() -> int:
    expected = os.getegid()
    return next(group for group in os.getgroups() if group != expected)
...
class CapabilityBundleTests(unittest.TestCase):
    ...
    def test_wrong_group_terminal_receipt_replay_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = pathlib.Path(temporary)
            self.addCleanup(make_writable, root)
            facts, hashes = self.fixture(root)
            installed = self.invoke_install(root)
            self.assertEqual(installed.returncode, 0, installed.stderr)
            receipt = (
            ...
            os.chown(receipt, -1, wrong_group())
            self.assertEqual(receipt.stat().st_gid, wrong_group())
            replay = self.invoke_install(root)
            self.assertNotEqual(replay.returncode, 0, replay.stderr)
            self.assertIn(
                "activation receipt member identity disagreement", replay.stderr
            )
            self.assertTrue(
                (
                    root / "Users/Shared/InterviewCopilot/"
                    "verification-controller-a04"
                ).exists()
            )
            self.assertTrue(
                (
                    root / "etc/sudoers.d/"
                    "interviewcopilot-verification-controller-a04"
                ).exists()
            )
    ...
    def test_wrong_group_journal_replay_fails_before_mutation(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = pathlib.Path(temporary)
            self.addCleanup(make_writable, root)
            facts, hashes = self.fixture(root)
            ...
            journal = (
                root / "Users/Shared/InterviewCopilot/"
                "verification-controller-a04-receipts/"
                "P00-V2-CAP-A04-activation.in-progress"
            )
            controller_identity = controller_root.lstat()
            sudoers_sha = sha256(sudoers)
            journal_sha = sha256(journal)
            os.chown(journal, -1, wrong_group())
            self.assertEqual(journal.stat().st_gid, wrong_group())
            replay = self.invoke_install(root)
            self.assertNotEqual(replay.returncode, 0, replay.stderr)
            self.assertIn(
                "activation journal member identity disagreement", replay.stderr
            )
            self.assertTrue(controller_root.exists())
            self.assertEqual(controller_root.lstat().st_ino, controller_identity.st_ino)
            self.assertTrue(sudoers.exists())
            self.assertEqual(sha256(sudoers), sudoers_sha)
            self.assertTrue(journal.exists())
            self.assertEqual(sha256(journal), journal_sha)
            self.assertFalse(
                (
                    journal.parent / "P00-V2-CAP-A04-activation.json"
                ).exists()
            )
            self.assert_fixture_unchanged(root, facts, hashes)
    ...
    def test_request_writer_and_handoff_have_exact_boundaries(self) -> None:
        ...
        self.assertIn(f"source_root={BUNDLE}", handoff)
        self.assertNotIn("verification-controller-bootstrap/", handoff)
        self.assertNotIn("P00-V2-CAP-A03-RECOVERY-", handoff)
        self.assertNotIn(
            "source_root="
            f"{BUNDLE.parent / 'P00-V2-CAP-A04'}\n",
            handoff,
        )
