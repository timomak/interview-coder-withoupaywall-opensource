...
if [[ -e "$install_root" ]]; then
  ...
  [[ -d "$install_root" && -d "$previous_metadata_root" ]] || {
    print -u2 "installed A01 upgrade identity is incomplete"
    exit 65
  }
...
if [[ -e "$sudoers_target" ]]; then
  ...
  if (( upgrading == 1 )); then
    [[ "$legacy_sudoers_sha" == "$previous_sudoers_sha" ]] || {
      print -u2 "installed A01 sudoers identity disagreement"
      exit 65
    }
...
print "installed P00-V2-CAP-A02 with exact empty-argument authorization"
